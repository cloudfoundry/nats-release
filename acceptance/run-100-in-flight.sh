...
( set -e;  bosh -n -d nats deploy $TESTDIR/manifest.yml -o $TESTDIR/100-max-flight.yml )
...
( set -e;  bosh -n -d nats deploy $TESTDIR/manifest.yml -o $TESTDIR/100-max-flight.yml -o $TESTDIR/replace-with-dev.yml )
